...
public class SqlServerProvider : DbProviderBase
{
	...
	{ 
		SqlHelper = new Parser.SqlServerSelectParser(this);
	}

	/// <summary>
	///		Crea la conexión
	/// </summary>
	protected override IDbConnection GetInstance() => new SqlConnection(ConnectionString.ConnectionString);
	...
	protected override IDataParameter ConvertParameter(ParameterDb parameter)
	{
		...
		if (parameter.IsText)
			return new SqlParameter(parameter.Name, SqlDbType.NText);
		...
		if (parameter.Value is string)
			return new SqlParameter(parameter.Name, SqlDbType.NVarChar, parameter.Length);
	...
	/// </summary>
	public override long BulkCopy(IDataReader reader, string table, Dictionary<string, string> mappings, 

...
{   
	...
	/// </summary>
	public object? ExecuteScalar(QueryModel query) => PrepareCommand(query).ExecuteScalar();
	...
	/// </summary>
	private int GetTimeout(TimeSpan? timeout) => (int) (timeout ?? TimeSpan.FromMinutes(1)).TotalSeconds;
	...
	private CommandType ConvertCommandType(QueryModel.QueryType type)
	{
		return type switch
				{
					QueryModel.QueryType.StoredProcedure => CommandType.StoredProcedure,
					QueryModel.QueryType.Table => CommandType.TableDirect,
					_ => CommandType.Text
				};
	}
	...
	private QueryModel.QueryType ConvertCommandType(CommandType type)
	{
		return type switch
				{
					CommandType.StoredProcedure => QueryModel.QueryType.StoredProcedure,
					CommandType.TableDirect => QueryModel.QueryType.Table,
					_ => QueryModel.QueryType.Text
				};
	}

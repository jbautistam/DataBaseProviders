...
internal class MySqlSchemaReader
{
	...
	///		Obtiene el esquema
	/// </summary>
	internal async Task<SchemaDbModel> GetSchemaAsync(MySqlProvider provider, SchemaOptions options, TimeSpan timeout, CancellationToken cancellationToken)
	{
		SchemaDbModel schema = new();

			// Carga los datos del esquema
			using (MySqlProvider connection = new(provider.ConnectionString))
			{
